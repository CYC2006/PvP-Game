...
# --- Packet types ---
PKT_JOIN   = 0x01   # client → server: request to join
PKT_JOINED = 0x02   # server → client: assigned player_id
PKT_CMD    = 0x03   # client → server: movement + shoot command
PKT_STATE  = 0x04   # server → client: full game state

# PKT_JOIN:   | type(1) |
# PKT_JOINED: | type(1) | player_id(1) |
# PKT_CMD:    | type(1) | player_id(1) | move_x(f) | move_y(f) | shooting(B) | aim_x(f) | aim_y(f) |
# PKT_STATE:  | type(1) | tick(I)
